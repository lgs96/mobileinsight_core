...
class ObjectLatencyAnalyzer(Analyzer):
    ...
    def replay_record(self, is_tcp, object_info):
        ...
        for time, event in enumerate(self.whole_record):
            ...
            diff_buffer = (self.whole_record[time - 1][3] - event_buffer)  
            if diff_buffer != 0 and diff_buffer < 0.9*event_grant:
                event_tx = event_grant
            
            if event_buffer < self.whole_record[time - 1][3]:
                event_tx = min(event_tx, self.whole_record[time - 1][3])
            ...
            if event_buffer > 5000 and object_state == object_idle:   
                ...
                try:
                    trigger_size = (int)(object_info[self.num_of_objects-1][0])
                    object_size = (int)(object_info[self.num_of_objects-1][1])
                    booster_size = (int)(object_info[self.num_of_objects-1][2])
                except:
                    return
            ...
            if event_buffer  > self.whole_record[time - 1][3] + 1000 and trigger_cert == 1:
                bullet_init_start = time - 1  
                print('Object injected time: ', bullet_init_start, self.num_of_objects) 
                trigger_cert = 2
            ...
            if (event_buffer < 500 and event_grant < 500) and object_state == object_tx:
                ...
                if idle_stack >= idle_time:
                    ...
                    if object_tx_byte > object_size/2:
                        idle_stack = 0
                        ...
                        #object_pure_tx_list.append([bullet_tx_start, time - 5])
            ...
            # current implementation uses interval between objects to measure performance.. 
            # however in the future, we can sense addition of buffer (addition of objects or dummy) to sense end of objects!   
            if event_tx < 300:
                object_idle_stack += 1
            else:
                object_idle_stack = 0
            ...
            if event_grant > 0 or event_tx > 0 or event_buffer > 0:
                ...
                if object_tx_byte >= trigger_size and trigger_on_tx == True:
                    trigger_on_tx = False
                    bullet_tx_start = time 
                    if self.num_of_objects != 0 and bullet_init_start != -1:
                        object_trigger_list.append([bullet_init_start, time])
                        print('End of trigger: ', self.num_of_objects, time - bullet_init_start)
                    else:
                        trigger_not_act = True
                if trigger_not_act == True and object_tx_stack >= 3 and bullet_init_start != -1:    
                    trigger_not_act = False
                    bullet_tx_start = time-2 
                    object_trigger_list.append([bullet_init_start, time-2])
                    print('End of trigger non act: ', self.num_of_objects, time - bullet_init_start)
                if object_tx_byte >= object_size + trigger_size and object_on_tx == True:
                    #print('Comminnnng!!')
                    if object_idle_stack >= 10:
                        object_idle_stack = 0
                        object_on_tx = False
                        if self.num_of_objects != 0:
                            object_pure_tx_list.append([bullet_tx_start, time - 10])
                            self.object_latency.append([object_trigger_list, [], object_pure_tx_list])
                            self.object_end_list.append(time)
                        print('End of object w/o booster', self.num_of_objects, object_trigger_list, object_pure_tx_list)
                    if object_tx_byte >= object_size*1.048 + trigger_size and event_buffer <= booster_size + 1000:
                        object_on_tx = False
                        if self.num_of_objects != 0:
                            object_pure_tx_list.append([bullet_tx_start, time])
                            self.object_latency.append([object_trigger_list, [], object_pure_tx_list])
                            self.object_end_list.append(time)
                        print('End of object w booster', self.num_of_objects, object_trigger_list, object_pure_tx_list)
                if object_tx_byte >= object_size + trigger_size + booster_size and booster_on_tx == True and object_on_tx == False:
                    if object_idle_stack >= 10:
                        object_idle_stack = 0
                        booster_on_tx = False
                        object_state = object_idle
                        print('End of booster ', self.num_of_objects)
                    ## intersect 
                    if object_tx_byte >= object_size*1.048 + trigger_size + booster_size*1.048:
                        object_idle_stack = 0
                        booster_on_tx = False
                        object_state = object_idle
                        print('End of booster ', self.num_of_objects)
    ...
    def calc_delay (self, start_time = 100, fin_time = 100000, object_info = []):
        self.replay_record(False, object_info)
    ...
    def analyze_object_delay(self):
        ...
        for i in range(self.num_of_objects):
            ...
            try:
                ...
                init_latency = []
                for j in init:
                    init_latency.append(self.__f_time_diff_object(j[0], j[1]) - 1)
                # sched
                sched_latency = []
                for j in sched:
