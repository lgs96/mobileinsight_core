...
class LteRrcAnalyzer(ProtocolAnalyzer):
    ...
    def create_state_machine(self):
        ...
        state_machine = {'RRC_IDLE': {'RRC_CRX': idle_to_crx},
                         ...
                         'RRC_LDRX': {'RRC_CRX': ldrx_to_crx, 'RRC_IDLE': crx_to_idle}}
    ...
    def __rrc_filter(self, msg):
        ...
        if msg.type_id == "LTE_RRC_OTA_Packet":
            ...
            if self.state_machine.update_state(xml_msg):
                ...
                self.log_info("Timestamp: " + str(xml_msg.timestamp)+" rrc state: " +str(log_item_dict['SysFrameNum/SubFrameNum']) +" "+str(self.state_machine.get_current_state()))

            #for field in xml_msg.data.iter('field'):
            #    print(field.get('name'))
            tic = time.process_time()
            self.__callback_rrc_conn(xml_msg, log_item_dict)
            ...
            for item in log_item_dict['Records']:
                ...
                if self.state_machine.update_state(raw_msg):
                    self.log_info("Timestamp: " + str(log_item_dict['timestamp'])+" rrc state: " + str(item['SFN']*10 + item['Sub-FN']) +" "+ str(self.state_machine.get_current_state()))
                    event = Event(msg.timestamp, 'rrc state', str(self.state_machine.get_current_state()))
                    self.send_to_coordinator(event)
    ...
    def __callback_drx(self, msg):
        ...
        for item in msg['Records']:
            #self.log_info("drx state: " + str(item['SFN']*10 + item['Sub-FN']) +" "+ str(item['CDRX Event']))
            if item['CDRX Event'] == "INACTIVITY_TIMER_START":
                drx_state['DRX state'] = "INACTIVITY_TIMER_START"#CRX
                self.broadcast_info('DRX', drx_state)
            elif item['CDRX Event'] == "INACTIVITY_TIMER_END":
                drx_state['DRX state'] = "INACTIVITY_TIMER_END"#CRX
                self.broadcast_info('DRX', drx_state)
            elif item['CDRX Event'] == "LONG_CYCLE_START":
                drx_state['DRX state'] = "LONG_CYCLE_START"#LONG_DRX
                self.broadcast_info('DRX', drx_state)
            elif item['CDRX Event'] == "SHORT_CYCLE_START":
                drx_state['DRX state'] = "SHORT_CYCLE_START"#SHORT_DRX
                self.broadcast_info('DRX', drx_state)
            #self.log_info("drx state: " + str(item['SFN']*10 + item['Sub-FN']) +" "+ str(drx_state))
    ...
    def __callback_rrc_reconfig(self, msg):
        ...
        for field in msg.data.iter('field'):
            ...
            #print(field.get('show'))
            ##goodsol
            if field.get('name') == "lte-rrc.rrcConnectionReconfigurationComplete_element":
                self.log_info("Timestamp: " + str(msg.timestamp)+" rrc state: "+str(self.state_machine.get_current_state())+ " rrcConnectionReconfigurationComplete!")
    ...
    def __callback_rrc_conn(self, msg, raw):
        """
        ...
        for field in msg.data.iter('field'):
            if field.get('name') == 'lte-rrc.rrcConnectionRequest_element':
                self.log_info("Timestamp: " + str(msg.timestamp)+" rrc state: " +str(raw['SysFrameNum/SubFrameNum']) +" "+str(self.state_machine.get_current_state())+ " rrcConnectionRequest!")
            ...
            if field.get('name') == "lte-rrc.rrcConnectionSetupComplete_element":
                ...
                drx_state['Timestamp'] = str(msg.timestamp)
                self.broadcast_info('DRX', drx_state)
                #self.log_info('DRX'+str(drx_state)+', sfn: '+ str(raw['SysFrameNum/SubFrameNum']//10240)+' '+str(raw['SysFrameNum/SubFrameNum']%10240))
