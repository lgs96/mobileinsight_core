...
class ObjectLatencyAnalyzer(Analyzer):
        ...
        for time, event in enumerate(self.whole_record):
            ...
            if event_buffer >= 60 and state == -1:
                if is_tcp:
                    ...
                    if event_buffer >= 1300:
                        ...
                        sched_start = time
            ## init event
            if event_buffer > 5000 and state == waiting:     
            ...
            if event_grant > 0 or event_tx > 0 or event_buffer > 0:
                print(time//10240, time%10240, ' Granted size: ', event_grant, ' Txed size: ', event_tx, 
                      ...
                      ' Mod order: ', self.mod_record[time], ' Coding: ', self.coding_record[time],' State: ', state)
            ## Cumulate grant size
            if time==0:
    ...
    def calc_delay (self, start_time = 100, fin_time = 100000):
        ...
        self.sr_point = [x[0] - start_point for x in self.sr[sr_start:sr_end]]
        self.sr_end_point = [x[1] - start_point for x in self.sr[sr_start:sr_end]] 
