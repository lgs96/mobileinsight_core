                        ...
                        for sample in log_item['Subpackets'][i]['Samples']:
                            ...
                            for lcid in sample['LCIDs']:
                                ...
                                if self.last_bytes[idx] > total_bytes:
                                    ...
                                    self.log_info(str(log_item['timestamp']) + ' Sent bytes: '+ str(sent_bytes))
                                    ...
                                    while len(self.buffer[idx]) > 0 and sent_bytes > 0:
                                        ...
                                        if pkt[1] <= sent_bytes:
                                            ...
                                            sent_bytes -= pkt[1]
                                            self.log_info(str(log_item['timestamp']) + " UL_PKT_DELAY: " + str(pkt_delay)+ ' Sent bytes: '+ str(sent_bytes))
                                            bcast_dict = {}
                                            bcast_dict['timestamp'] = str(log_item['timestamp'])
